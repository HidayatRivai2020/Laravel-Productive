...
$(document).ready(function() {
    // Check for URL parameters to restore selections
    const urlParams = new URLSearchParams(window.location.search);
    const categoryId = urlParams.get('category_id');
    const contentId = urlParams.get('content_id');
    const objectiveId = urlParams.get('objective_id');
    
    // Restore selections if parameters exist
    if (categoryId) {
        restoreSelections(categoryId, contentId, objectiveId);
    }
    
    // Function to restore previous selections
    function restoreSelections(categoryId, contentId, objectiveId) {
        // Set category selection
        $('#categorySelect').val(categoryId).trigger('change');
        
        // Wait for category change to complete, then set content
        if (contentId) {
            setTimeout(function() {
                $('#contentSelect').val(contentId).trigger('change');
                
                // Wait for content change to complete, then set objective and load tasks
                if (objectiveId) {
                    setTimeout(function() {
                        $('#objectiveSelect').val(objectiveId).trigger('change');
                    }, 300);
                }
            }, 300);
        }
        
        // Clean URL after restoring selections
        setTimeout(function() {
